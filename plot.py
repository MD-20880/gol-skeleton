...
benchmark_data = pd.read_csv('results3.csv', header=0, names=['name', 'time', 'range'])
...
benchmark_data['threads'] = benchmark_data['name'].str.extract('Gol/512x512x500-(\d+)-\d+').apply(pd.to_numeric)
benchmark_data['cpu_cores'] = benchmark_data['name'].str.extract('Gol/512x512x500-\d+-(\d+)').apply(pd.to_numeric)
