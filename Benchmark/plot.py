...
benchmark_data['Num_Workers'] = benchmark_data['name'].str.extract('Gol/\d+x\d+x\d+x(\d+)-\d+').apply(pd.to_numeric)
benchmark_data['cpu_cores'] = benchmark_data['name'].str.extract('Gol/\d+x\d+x\d+x\d+-(\d+)').apply(pd.to_numeric)

print(benchmark_data)

# Plot a bar chart.
ax = sns.barplot(data=benchmark_data, x='Num_Workers', y='time')

# Set descriptive axis lables.
ax.set(xlabel='Worker Number', ylabel='Time taken (s)')
